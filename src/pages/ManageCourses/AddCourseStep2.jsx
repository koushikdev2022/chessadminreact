...
const AddCourseStep2 = ({ onBack, levelId, course_id }) => {
  ...
  const updateForm = (idx, changes) => {
    setModuleForms((prev) =>
      prev.map((f, i) => (i === idx ? { ...f, ...changes } : f))
    );
  };
  ...
  const handleModuleSearch = (idx, searchTerm) => {
    ...
    dispatch(
      searchModule({ course_level_id: levelId, module_search: searchTerm })
    ).then((res) => {
      const options =
        res?.payload?.res?.map((mod) => ({
          value: mod.id,
          label: mod.topic_name,
        })) || [];
  ...
  const handleLessionSearch = (idx, moduleId, searchTerm) => {
    ...
    dispatch(
      searchLession({ module_id: moduleId, lession_search: searchTerm })
    ).then((res) => {
      const options =
        res?.payload?.res?.map((les) => ({
          value: les.id,
          label: les.module_name,
        })) || [];
  ...
  const onSubmit = (data) => {
    ...
    const errors = moduleForms.map((form) => {
      ...
      if (!form.module_duration_n || !form.module_duration)
        err.duration = "Module duration is required";
      if (!form.files || form.files.length === 0)
        err.files = "At least one PDF file is required";
      return err;
    });
    ...
    const modulesData = moduleForms.map((form) => ({
      ...
      module_duration:
        `${form.module_duration_n} ${form.module_duration}`.trim(),
      files: form.files || [],
    }));
    ...
    dispatch(addCourseStep2(formData)).then((res) => {
      console.log("res", res);
      ...
      <div className="min-h-screen bg-gray-100">
        <div className="max-w-full mx-auto p-6 bg-white shadow rounded-xl">
          <div className="flex justify-between items-center">
            ...
            <button
              type="button"
              className="border border-blue-800 text-blue-800 w-[15rem] px-3 py-1 rounded hover:bg-gray-100 mb-6"
              onClick={handleAddModuleForm}
            >
              + Add Module
            </button>
          </div>
          <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="flex justify-between gap-3 mb-4">
                  <div className="w-full">
                    <Select
                      ...
                      isLoading={loading}
                      noOptionsMessage={() =>
                        form.moduleSearchTerm
                          ? "No modules found"
                          : "Type to search"
                      }
                    />
                    {formErrors[idx]?.module && (
                  ...
                  <div className="flex justify-start items-center gap-4 mb-4">
                    <div>
                      <input
                        ...
                        onChange={(e) =>
                          updateForm(idx, { module_duration_n: e.target.value })
                        }
                      />
                    </div>
                    <div>
                      <select
                        ...
                        onChange={(e) =>
                          updateForm(idx, { module_duration: e.target.value })
                        }
                      >
            ...
            <div className="flex justify-end gap-4 mb-6">
              <button
                onClick={onBack}
                className="border border-blue-800 text-blue-800 px-5 py-2 rounded hover:bg-gray-100"
              >
                Cancel
              </button>
              <button
