...
import { addCourseStep1, courseLevelDropdown, courseTagsDropdown } from "../../Reducer/CourseSlice";
...
const AddCourseStep1 = ({ onNext }) => {
    ...
    const tagOptions = courseTagsDropdownData?.list?.map((tag) => ({
        value: tag?.id,
        label: tag?.title,
    })) || [];
    ...
    const onSubmit = (data) => {
        ...
        if (!match) {
            toast.error("Invalid duration format. Use something like '3 months' or '2weeks'.");
            return;
        }
        ...
        formData.append(
            "student_will_learn",
            JSON.stringify(
                learningOutcomes?.filter((outcome) => outcome?.trim() !== "")?.map((outcome) => ({ name: outcome?.trim() }))
            )
        );
            ...
            <div className="min-h-screen bg-gray-100">
                <div className="max-w-full mx-auto p-6 bg-white shadow rounded-xl">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-medium mb-1">Course Subtitle</label>
                            <input
                                type="text"
                                placeholder="Enter Course Subtitle"
                                className="w-full border rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                {...register("course_sub_title", { required: "Course Sub Title is required" })}
                            />
                            {errors?.course_sub_title && <p className="text-red-500 text-sm">{errors.course_sub_title.message}</p>}
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <div>
                                <label className="block text-sm font-medium mb-1">Course Tags</label>
                                <Controller
                                    name="course_tags"
                                    control={control}
                                ...
                                />
                                {errors?.course_tags && <p className="text-red-500 text-sm">{errors.course_tags.message}</p>}
                            </div>

                            <div>
                                <label className="block text-sm font-medium mb-1">Course Level</label>
                                <select className="w-full border rounded px-4 py-2" {...register("course_level_id", { required: "Course Level is required" })}>
                                ...
                                {errors?.course_level_id && <p className="text-red-500 text-sm">{errors.course_level_id.message}</p>}
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    {...register("course_duration_input", { required: "Course Sub Title is required" })}
                                />
                                {errors?.course_duration_input && <p className="text-red-500 text-sm">{errors.course_duration_input.message}</p>}
                            </div>
                        ...
                        <div className="mb-4">
                            ...
                            <textarea
                                ...
                                {...register("course_description", { required: "Course Sub Title is required" })}
                            ></textarea>
                            {errors?.course_description && <p className="text-red-500 text-sm">{errors.course_description.message}</p>}
                        </div>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex flex-col md:flex-row gap-4">
                                ...
                                <label className="w-full md:w-1/2 border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center text-center bg-blue-50 cursor-pointer hover:border-blue-400">
                                    ...
                                    <span className="text-sm text-gray-600">
                                        Drag and Drop Image or <span className="text-blue-600 font-medium">Browse</span><br />
                                        <span className="text-xs text-gray-400">SVG, PNG, JPG or JPEG (max 18 MB)</span>
                                    </span>
                                </label>
                                ...
                                {file && (
                                    <div className="relative flex-1 border rounded-lg p-4">
                                        <div className="flex justify-between items-center mb-2">
                                            ...
                                            <button onClick={removeFile} className="text-gray-500 hover:text-red-600 text-lg">
                                                ×
                                            </button>
                                        </div>
                            ...
                            <div className="flex justify-end gap-4 mb-6">
                                <button className="border border-blue-800 text-blue-800 px-5 py-2 rounded hover:bg-gray-100">
                                    Cancel
                                </button>
                                <button type="submit" className="bg-blue-800 text-white px-6 py-2 rounded hover:bg-blue-900">
                                    {addCourseStep1loading ? "Saving please wait..." : "Save and Continue →"}
                                </button>
                            </div>
