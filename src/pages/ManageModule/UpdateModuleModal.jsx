...
import { getModules, moduleDetails, updateModule } from "../../Reducer/ModuleSlice";
import { toast } from "react-toastify";

const UpdateModuleModal = ({ openUpdateModuleModal, setOpenUpdateModuleModal, moduleId }) => {
    ...
    return (
        <>
            <Modal show={openUpdateModuleModal} onClose={() => setOpenUpdateModuleModal(false)}>
                <Modal.Header>Update This Module</Modal.Header>
                    ...
                    <Modal.Body>
                        <div className="space-y-4 max-w-full md:max-w-lg mx-auto px-2 sm:px-4">
                            <div>
                                ...
                                />
                                {errors?.module_name && <p className="text-red-500 text-sm">{errors.module_name.message}</p>}
                            </div>
                            <div>
                                ...
                                {errors?.module_short_name && (
                                    <p className="text-red-500 text-sm">{errors.module_short_name.message}</p>
                                )}
                            </div>
                            <div>
                                <Label htmlFor="module_description" value="Module Description" />
                                <Textarea
                                    id="module_description"
                                    rows={3}
                                    {...register("module_description", { required: "Description is required" })}
                                />
                                {errors?.module_description && (
                                    <p className="text-red-500 text-sm">{errors.module_description.message}</p>
                                )}
                            </div>
                        </div>
                    </Modal.Body>
